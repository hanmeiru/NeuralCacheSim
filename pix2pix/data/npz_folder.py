...
def make_dataset_bench(dir, max_dataset_size=float("inf")):
    '''
    input is a dir with dirs of benchmarks
    '''

    # ex. of dir: /tank/projects/neuralcachesim/sparse-matrices/size1/64set-12way_sparse/SPEC/TRAIN/FULL
    npz_files = []
    assert os.path.isdir(dir), '%s is not a valid directory' % dir
    for bench in os.listdir(dir):
        bench_path = os.path.join(dir,bench)
        for root, _, fnames in sorted(os.walk(bench_path)):
            for fname in fnames:
                if is_npz_file(fname):
                    path = os.path.join(root, fname)
                    npz_files.append(path)
...
if __name__=='__main__':
    make_dataset('/tank/projects/neuralcachesim/sparse-matrices/size1/64set-12way_sparse/SPEC/TRAIN/FULL')
    make_dataset('/tank/projects/neuralcachesim/sparse-matrices/size1/64set-12way_sparse/SPEC/TRAIN/MISS')
