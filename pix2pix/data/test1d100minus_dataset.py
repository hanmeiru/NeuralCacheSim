...
class Test1D100MinusDataset(BaseDataset):
    """
    ...
    def __init__(self, opt, config='', benchmark=''):
        ...
        self.dir_A = os.path.join(opt.dataroot,'TESTALL/FULL','',benchmark)  # create a path '/dataroot/TEST/FULL/bench
        self.dir_B = os.path.join(opt.dataroot,'TESTALL/MISS','',benchmark)   # create a path '/dataroot/TEST/MISS/bench
    ...
    def __getitem__(self, index):
        ...
        image_idx = float(A_path.split('txt_')[1].split('_')[0])
       
        # convert A and B from sparse matrix to ndarray to img
        sparse_A = load_npz(A_path) # load sparse matrix from path
        array_A = 100 - np.array(sparse_A.toarray(), dtype = np.uint8) #convert into array
        sparse_B = load_npz(B_path) # load sparse matrix from path
        array_B = 100 - np.array(sparse_B.toarray(), dtype = np.uint8) #convert into array
        ...
        return {'A': A, 'B': B, 'A_paths': A_path, 'B_paths': B_path, 'cache_set':cache_set, 'cache_way':cache_way, 'image_idx':image_idx}
