'''
Post-process by converting 3d image to 1d and unscaling (check data/test3d_dataset.py)
...
if __name__ == '__main__':
    ...
    testA_path = os.path.join(opt.dataroot, 'testA')
    testB_path = os.path.join(opt.dataroot, 'testB')
    ...
    # it will be saved under the dir of experiment name
    ...
    config_list = os.listdir(testA_path)
    ...
    for config in config_list:
        ...
        # ex. .../experiment_MISS/experiment0
        tank_experiment_name_dir = os.path.join(tank_experiment_MISS_dir,opt.name)
        # print(expetiment_name_dir)

        try:
            os.mkdir(tank_experiment_name_dir) 
        except: 
            pass
        bench_list = os.listdir(os.path.join(testA_path,config))
        ...
        for bench in bench_list:
            ...
            bench_dir = os.path.join(fakeB_dir,config,bench) # not in tank
            try:
                os.mkdir(bench_dir)
               # os.mkdir(tank_bench_dir)
            except:
                pass
            ...
            #         break
                ...
                # print('min before: ', model.fake_B.min()) # -1,1
                # print('max before: ',model.fake_B.max())
                ...
                # print('min after: ', fake_B_255.min()) # 0,255
                # print('max after: ',fake_B_255.max())

                # Post processing (for saving the image and hit-rate calculation):
                # Note: check test3d_dataset for how the arrays are scaled 
