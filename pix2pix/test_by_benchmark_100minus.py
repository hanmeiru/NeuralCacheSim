'''
Post-process by getting subtracted by 100 (check test1d100minus_dataset.py). 
    ...
    for config in config_list:
        ...
        for bench in bench_list:
            ...
            for i, data in enumerate(dataset):
                ...
                # no scale 
                unscaled_fake_B = 100-util.tensor2im(model.fake_B)[:,:,0] # tensor2im gives (512,512,3)
                unscaled_real_B = 100-util.tensor2im(model.real_B)[:,:,0]
                unscaled_real_A = 100-util.tensor2im(model.real_A)[:,:,0]
